...
// update, and its minor when at least one of them is a minor update. A forced run with nothing
// updated keeps bumping the minor. Run with `node --test test/` (npm run test:unit).
...
test('bumpLevel: patch only when every update is a patch update', () => {
    const cases = [
        ...
        {name: 'a forced run with nothing updated', updates: [], want: 'minor'}
    ];
